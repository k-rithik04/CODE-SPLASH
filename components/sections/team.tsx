  ...
  return (
    ...
    >
      ...
      <div ref={teamTrackRef} className="flex gap-8 md:gap-16 absolute left-[50%] items-center h-[70vh] top-[15vh]">
        {items.map((member) => (
          <div key={member.id} className="w-[280px] md:w-[320px] h-[380px] md:h-[420px] shrink-0 rounded-2xl flex flex-col overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.5)] bg-glass-bg backdrop-blur-[40px] border border-glass-border pointer-events-auto transition-transform hover:-translate-y-2 group">

            <div className="h-[65%] w-full bg-cover bg-center bg-no-repeat relative" style={{ backgroundImage: `url('${imageUrl(member.image_url)}')` }}>
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div>

              {member.linkedin_url && (
                <a href={member.linkedin_url} target="_blank" rel="noopener noreferrer" className="absolute bottom-4 right-4 text-white/90 transition-all duration-300 hover:scale-110 hover:text-orange z-10 drop-shadow-md">
